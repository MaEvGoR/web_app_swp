...
--Insert some data
COPY SystemUser(UserID, Mail, Hash, Name, Surname) FROM 'C:\swpdb\SystemUser.dat';

COPY DOEmember(UserID) FROM 'C:\swpdb\DOEmember.dat';

COPY TeacherAssistantOfTheCourse(CourseID, TeacherAssistantID) FROM 'C:\swpdb\TA.dat';

COPY StudentOfTheCourse(StudentID, CourseID)  FROM 'C:\swpdb\Student.dat';

COPY  Course(CourseID, Name, ProfessorID) FROM 'C:\swpdb\Course.dat';

COPY  Survey(SurveyID, Name, CreationTime, ExpirationTime, Description, ConsideredUserID)  FROM 'C:\swpdb\Survey.dat';

COPY Question(SurveyID, QuestionNumber, Type, QuestionText) FROM 'C:\swpdb\Question.dat';

COPY AnswerOption(SurveyID, Text, QuestionNumber, OptionNumber) FROM 'C:\swpdb\AnswerOption.dat';

COPY FilledSurvey(FilledSurveyID, ConstrainedBySurveyID) FROM 'C:\swpdb\FilledSurvey.dat';

COPY Answer(FilledSurveyID, AnswerNumber, Type, ChosenOption, AnswerText) FROM 'C:\swpdb\Answer.dat';

COPY UserFillInSurvey(UserID, SurveyID) FROM 'C:\swpdb\UserFillInSurvey.dat';
